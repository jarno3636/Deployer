...
export function Deployer() {
  ...
  return <main className="shell">
    ...
    <section className="card stepCard"><div className="stepHead"><span className="stepNo">07</span><div><h2>Deploy CCTP Bridge Registry V1</h2><p>Non-custodial safety registry for ScanArc bridging. User USDC goes directly to Circle CCTP V2; this contract has no token transfer, approval, burn, mint, or withdrawal functions.</p></div></div>
      <div className="details"><div><span>Circle TokenMessengerV2</span><code>{CCTP_V2.tokenMessengerV2}</code></div><div><span>Circle MessageTransmitterV2</span><code>{CCTP_V2.messageTransmitterV2}</code></div><div><span>Enabled at deployment</span><strong>{CCTP_MAINNET_CHAINS.map((c) => c.name).join(" · ")}</strong></div><div><span>Held disabled pending native-USDC verification</span><strong>{CCTP_PENDING_DOMAINS.length ? CCTP_PENDING_DOMAINS.map((c) => c.name).join(" · ") : "None"}</strong></div></div>
