...
import { cctpBridgeV2StandardJsonInput } from "../../../../lib/scanarc-cctp-bridge-v2-verification.generated";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = new URLSearchParams({
      ...
      contractname: "contracts/ScanArcCCTPBridgeV2.sol:ScanArcCCTPBridgeV2",
      compilerversion: COMPILER,
      sourceCode: cctpBridgeV2StandardJsonInput,
    ...
    if (!res.ok || String(json?.status) === "0") {
      return NextResponse.json(
        { error: json?.result || json?.message || `Blockscout verification failed (${res.status}).` },
        { status: 422 },
      );
    }

    return NextResponse.json({ ok: true, message: json?.result || json?.message || "CCTP Bridge V2 verification submitted." });
  } catch (error) {
