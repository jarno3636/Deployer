...
import { scanArcV6StandardJsonInput } from "../../../../lib/scanarc-v6-verification.generated";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = new URLSearchParams({
      ...
      contractname: "contracts/ScanArcRouterV6.sol:ScanArcRouterV6",
      compilerversion: COMPILER,
      sourceCode: scanArcV6StandardJsonInput,
    ...
    return NextResponse.json({ ok: true, message: json?.result || json?.message || "V6 verification submitted." });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "V6 verification submission failed." }, { status: 500 });
  }
}
