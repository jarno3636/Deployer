...
const unit = 'contracts/ScanArcRouterV5.sol';
const name = 'ScanArcRouterV5';
...
if (!artifact?.evm?.bytecode?.object) throw new Error('Missing ScanArcRouterV5 artifact');
fs.mkdirSync(path.join(root, 'lib'), { recursive: true });
fs.mkdirSync(path.join(root, 'artifacts'), { recursive: true });
const generated = `// AUTO-GENERATED FROM contracts/ScanArcRouterV5.sol. Do not edit.\nexport const routerAbi = ${JSON.stringify(artifact.abi, null, 2)} as const;\nexport const routerBytecode = ${JSON.stringify(`0x${artifact.evm.bytecode.object}`)} as \`0x\${string}\`;\nexport const compilerVersion = ${JSON.stringify(solc.version())} as const;\n`;
fs.writeFileSync(path.join(root, 'lib', 'scanarc-router.generated.ts'), generated);
fs.writeFileSync(path.join(root, 'artifacts', 'ScanArcRouterV5.abi.json'), JSON.stringify(artifact.abi, null, 2));
fs.writeFileSync(path.join(root, 'artifacts', 'ScanArcRouterV5.bin'), artifact.evm.bytecode.object);
fs.writeFileSync(path.join(root, 'artifacts', 'ScanArcRouterV5.standard-input.json'), JSON.stringify(input, null, 2));
